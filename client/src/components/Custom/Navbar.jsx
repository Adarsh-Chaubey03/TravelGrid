...
const Navbar = () => {
  ...
  return (
    <div>
      {/* Top Navbar */}
      <nav
        className={`box-border w-full fixed top-0 left-0 z-50 h-20 backdrop-blur-lg border-b transition-all duration-300 pr-4 sm:pr-6 pl-0 ${
          isDarkMode
            ? "bg-slate-900/90 backdrop-blur-md border-slate-700 text-white"
            : "bg-slate-900/95 backdrop-blur-lg border-white/20 text-white"
        }`}
      >
          ...
          <div className="hidden md:flex gap-3 items-center text-white font-medium">
            ...
            {isLoggedIn ? (
              ...
              <>
                <NavLink
                  to="/login"
                  className={`px-3 py-2 rounded-md font-semibold hover:scale-105 transition-all text-sm whitespace-nowrap ${
                    isDarkMode
