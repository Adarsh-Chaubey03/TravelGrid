...
import { FaSearch, FaMoon, FaSun, FaTimes, FaExclamationTriangle, FaSnowflake, FaTemperatureLow, FaTshirt, FaUmbrella, FaSnowman, FaWind, FaExchangeAlt, FaCrown, FaEye } from "react-icons/fa";
...
const WeatherWiseTravelPlanner = () => {
  ...
  const fetchWeatherData = async (location) => {
    ...
    try {
      const currentResponse = await fetch(`${BASE_URL}/weather/realtime?location=${loc}&units=${currentUnit}&apikey=${API_KEY}`);
      if (!currentResponse.ok) {
        const errorData = await currentResponse.json();
        throw new Error(errorData.message || "Location not found");
      }
      const currentData = await currentResponse.json();
      console.log("Current Weather Data:", JSON.stringify(currentData, null, 2)); // Detailed debug log
      setWeatherData(currentData);

      const forecastResponse = await fetch(`${BASE_URL}/weather/forecast?location=${loc}&timesteps=1d&units=${currentUnit}&apikey=${API_KEY}`);
      if (!forecastResponse.ok) {
        const errorData = await forecastResponse.json();
        throw new Error(errorData.message || "Forecast data not available");
      }
      const forecastData = await forecastResponse.json();
      console.log("Forecast Data:", JSON.stringify(forecastData, null, 2)); // Detailed debug log
      setForecastData(forecastData);

      const hourlyResponse = await fetch(`${BASE_URL}/weather/forecast?location=${loc}&timesteps=1h&units=${currentUnit}&apikey=${API_KEY}`);
      if (!hourlyResponse.ok) {
        const errorData = await hourlyResponse.json();
        throw new Error(errorData.message || "Hourly data not available");
      }
      const hourlyData = await hourlyResponse.json();
      console.log("Hourly Data:", JSON.stringify(hourlyData, null, 2)); // Detailed debug log
  ...
  const getWeatherIcon = (code) => {
    // Using a reliable weather icon set from OpenWeatherMap
    const iconMap = {
      0: "50d", // Unknown
      1000: "01d", // Clear
      1001: "04d", // Cloudy
    ...
    const iconCode = iconMap[code] || "50d";
  ...
  const renderCurrentWeather = () => {
    ...
    const wind = currentUnit === "metric" ? `${Math.round(weatherData.data.values.windSpeed)} km/h` : `${Math.round(weatherData.data.values.windSpeed * 0.621371)} mph`;
          ...
          <div className="location-info flex-1 min-w-[250px]">
            ...
            <div className="weather-main flex items-center mb-4">
              <img
                ...
                onError={(e) => {
                  console.error(`Failed to load current weather icon: ${e.target.src}`);
                  e.target.src = "https://via.placeholder.com/96x96?text=Weather";
                }}
              />
              <div>
            ...
            return (
              ...
              >
                ...
                <img
                  ...
                  onError={(e) => {
                    console.error(`Failed to load forecast icon: ${e.target.src}`);
                    e.target.src = "https://via.placeholder.com/48x48?text=Weather";
                  }}
                />
  ...
  const renderBestTimes = () => {
    ...
    const dayHours = hourlyData.timelines.hourly.filter((hour) => new Date(hour.time).toDateString() === selectedDate);

    const bestTimes = findBestTimes(dayHours);

    return (
      <div id="best-times-container" className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-8 shadow-md">
              ...
              <div key={timeSlot.time} className="best-time-card bg-pink-50 dark:bg-gray-700 rounded-lg p-4 relative">
                ...
                <div className="flex items-center">
                  <img
                    ...
                    alt={weatherDesc}
                    onError={(e) => {
                      console.error(`Failed to load best times icon: ${e.target.src}`);
                      e.target.src = "https://via.placeholder.com/40x40?text=Weather";
                    }}
                  />
                  <div className="text-gray-700 dark:text-gray-300">{weatherDesc}</div>
                </div>
                ...
                {index === 0 && <div className="best-time-badge"><FaCrown /></div>}
              </div>
            );
          })}
        </div>
  ...
  const renderDetailedView = () => {
    ...
    const selectedDate = new Date(selectedDayData.time).toDateString();
    const vis = currentUnit === "metric" ? `${Math.round(selectedDayData.values.visibility / 1000)} km` : `${Math.round(selectedDayData.values.visibility / 1609)} miles`;
    ...
    return (
      <div id="detailed-view" className="detailed-view bg-white dark:bg-gray-800 rounded-xl p-6 mb-8 shadow-md">
        ...
        <div id="hourly-forecast" className="hourly-forecast flex overflow-x-auto gap-4 mb-6 py-2 custom-scroll">
          {hourlyData.timelines.hourly
            .filter((hour) => new Date(hour.time).toDateString() === selectedDate)
            .map((hour) => (
              <div key={hour.time} className="hourly-card bg-pink-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                <div className="hourly-time font-medium mb-1">{formatTime(new Date(hour.time))}</div>
                <img
                  ...
                  onError={(e) => {
                    console.error(`Failed to load hourly icon: ${e.target.src}`);
                    e.target.src = "https://via.placeholder.com/40x40?text=Weather";
                  }}
                />
                <div className="hourly-temp font-bold">{`${Math.round(hour.values.temperature)}°`}</div>
        ...
        <div id="recommendations-list" className="space-y-4">
          {recommendations.map((rec, index) => (
            ...
            >
              ...
              <div className="rec-text text-gray-700 dark:text-gray-200">{rec.text}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <button 
        id="theme-toggle" 
        className="theme-toggle fixed top-4 right-4 p-3 rounded-full bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 z-10 transition-all duration-300 hover:scale-110"
        onClick={toggleTheme}
      >
        {isDarkMode ? (
      ...
      <header className="text-center mb-8 bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md animate-fade-in">
        <h1 className="text-4xl font-bold text-primary dark:text-accent mb-2">WeatherWise Travel Planner</h1>
        <p className="text-lg text-secondary dark:text-pink-300 mb-6">Plan your perfect trip based on weather forecasts</p>
        ...
        <div className="unit-toggle-container flex justify-center">
          <div className="unit-toggle flex items-center bg-gray-100 dark:bg-gray-700 rounded-full p-1 shadow-inner">
            <div
              className={`unit-toggle-option flex items-center justify-center w-12 h-10 rounded-full cursor-pointer transition-all duration-300 ${
                currentUnit === "metric" 
      ...
      {loading && (
        <div id="loading" className="text-center py-12">
          ...
          <p className="text-lg text-secondary dark:text-gray-300">Fetching weather data...</p>
        </div>
      )}
